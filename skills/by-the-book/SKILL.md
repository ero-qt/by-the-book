...
Message: the subject says what the commit does to the repo, never what the code now does. Imperative, lowercase, no articles, 50 characters or fewer. When `git log` shows the repo does it another way, the repo wins. Body unless `--no-body`: why, and what it rejects, wrapped at 72, referencing `#<N>`. No trailers, no sign-offs.
...
- A commit subject that describes the code's behavior instead of the change, or one with a capital or an article, a PR subject without its issue number, an issue title that names a fix
